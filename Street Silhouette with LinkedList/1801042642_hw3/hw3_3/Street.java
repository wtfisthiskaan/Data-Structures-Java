...
public class Street{
	...
	private LDLinkedList[] buildings=new LDLinkedList[2];
	//length of street 
	private int length;
	...
	*/
	public Street(int length,int maxBuildingNum){
		this.length=length;
		//buildings=
		buildings[0]=new LDLinkedList();
		buildings[1]=new LDLinkedList();
	}
	/** Add Function.
	...
	public void add(Building b,int side){
		...
		if((side==0 || side==1) && (b.getPosition()+b.getLength()<=length) && (b.getPosition()>=0) && (b.getLength()<=length)){
			if(side==0){
				...
				for(i=0;i<buildings[0].size();i++){
					//check if there is overlap situation
					if(!((buildings[side].get(i).getPosition()>b.getLength()+b.getPosition()) 
						|| (buildings[side].get(i).getPosition()+buildings[side].get(i).getLength()<b.getPosition())))
						throw new IllegalArgumentException("error message");//
				}

				buildings[side].add(b);
				//increment count
			}
			else{//check for building overlap
				for(i=0;i<buildings[1].size();i++){
					//check if there is overlap situation
					if(!((buildings[side].get(i).getPosition()>=b.getLength()+b.getPosition()) 
						|| (buildings[side].get(i).getPosition()+buildings[side].get(i).getLength()<=b.getPosition())))
						throw new IllegalArgumentException("error message");;//
	...
	*/
	public void remove(int index,int side){
		if(side==0){
			if(index>=buildings[0].size())
				throw new IllegalArgumentException("Out Of Index");
			buildings[0].remove(index);
		}
		else if(side==1){
			if(index>=buildings[1].size())
				throw new IllegalArgumentException("Out Of Index");
			buildings[1].remove(index);
	...
	*/
	public void list_playgrounds(){
		for(int i=0;i<buildings[0].size();i++){
			if(buildings[0].get(i).getType()=="Playground")
			System.out.println(buildings[0].get(i).focus());
		}
		for(int i=0;i<buildings[1].size();i++){
			if(buildings[1].get(i).getType()=="Playground")
			System.out.println(buildings[1].get(i).focus());
		}
	}
	...
	*/
	public void displayRemainingLands(){
		int total=0;
		for(int i=0;i<buildings[0].size();i++){
			total+=buildings[0].get(i).getLength();
		}
		for(int i=0;i<buildings[1].size();i++){
			total+=buildings[1].get(i).getLength();
		}
	...
	*/
	public void displayBuildings(){
		...
		for(int i=0;i<buildings[0].size();i++){
			...
			buildings[0].get(i).getInfo();
			System.out.println("-----------");

		}
		for(int i=0;i<buildings[1].size();i++){
			System.out.println("Building No: (Back Side)"+i);
			buildings[1].get(i).getInfo();
			System.out.println("-----------");
		}
	}
	...
	*/
	public void calculateLength(String type){
		int total=0;
		for(int i=0;i<buildings[0].size();i++){
			if(buildings[0].get(i).getType()==type){
		...
		for(int i=0;i<buildings[1].size();i++){
			if(buildings[1].get(i).getType()==type){
				System.out.println("ba");
				total+=buildings[1].get(i).getLength();
			}
	...
	public void drawSkylineSil(){
		...
		//fill front
		for(i=0;i<buildings[0].size();i++){
			pos=buildings[0].get(i).getPosition();
			len=buildings[0].get(i).getLength();
			height=buildings[0].get(i).getHeight();
		...
		//fill back
		for(i=0;i<buildings[1].size();i++){
			pos=buildings[1].get(i).getPosition();
			len=buildings[1].get(i).getLength();
			height=buildings[1].get(i).getHeight();
			for(j=pos;j<pos+len;j++){
